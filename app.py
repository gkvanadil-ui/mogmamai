...
# --- [로직 2: 모그 작가님 전용 어투 엔진 - 따님의 로직 100% 반영] ---
def ask_mog_ai(platform, user_in=""):
    client = openai.OpenAI(api_key=api_key)
    ...
    # 2️⃣ [플랫폼별] 특화 프롬프트 로직
    if platform == "인스타그램":
        system_p = f"{base_style} [📸 인스타그램 - 감성 일기 모드] 사진을 보자마자 마음이 따뜻해지는 문구로 시작할 것. 구성: [첫 줄 감성 문구] + [작가님의 제작 일기] + [작품 상세 정보] + [다정한 인사] + [해시태그]. 줄바꿈을 아주 넉넉히 해서 가독성을 높이고, 해시태그는 10개 내외로 달기."
    elif platform == "아이디어스":
        system_p = f"{base_style} [🎨 아이디어스 - 정성 가득 모드] 작가님의 수고와 정성이 고객에게 고스란히 전달되게 할 것. 매우 잦은 줄바꿈과 짧은 문장 위주. '한 땀 한 땀', '밤새 고민하며' 등 정성이 듬뿍 느껴지는 단어 사용."
    elif platform == "스토어":
        system_p = f"{base_style} [🛍️ 스마트스토어 - 친절 정보 모드] 필요한 정보를 한눈에 보기 좋게 정리하되, 딱딱하지 않게 설명할 것. 구분선(⸻)을 사용하여 소재, 사이즈, 관리법을 명확히 구분. 전문적이면서도 다정한 '상담원' 같은 느낌으로 신뢰감 주기."
    elif platform == "상담":
        system_p = f"{base_style} [💬 상담소 - 고민 상담 전용 로직] 핸드메이드 작가들의 든든한 선배이자 다정한 동료 '모그 AI'. 엄마의 고민에 깊이 공감해주고 실질적 도움을 줄 것. 항상 작가님의 활동을 진심으로 응원하는 따뜻한 격려 멘트 필수."

    info = f"작품명:{st.session_state.m_name}, 소재:{st.session_state.m_mat}, 기간:{st.session_state.m_per}, 대상:{st.session_state.m_tar}, 상세:{st.session_state.m_det}"
    res = client.chat.completions.create(
        model="gpt-4o",
        messages=[{"role":"system","content":system_p},{"role":"user","content":f"정보: {info} / 요청사항: {user_in}"}]
    )
    # 💡 따님의 팁: 특수기호 제거 로직 적용
...
with tabs[0]: # 판매글 쓰기
    ...
    for k, v in st.session_state.texts.items():
        if v: st.text_area(f"📍 {k} 글이 완성되었어요^^", value=v, height=250, key=f"o_{k}")

with tabs[1]: # 📸 AI 자동 사진 보정 (3단계 로직)
    ...
    if up_img and st.button("✨ 보정 시작하기"):
        with st.spinner("우리 작품을 화사하게 만드는 중이에요..."):
            ...
            col1.image(up_img, caption="보정 전", use_container_width=True)
            col2.image(e_img, caption="보정 후", use_container_width=True)
            buf = io.BytesIO(); e_img.save(buf, format="JPEG")
            st.download_button("📥 보정된 사진 저장하기", buf.getvalue(), "mogs_fixed.jpg", "image/jpeg")

with tabs[2]: # 💬 고민 상담소 (3번 로직 반영)
