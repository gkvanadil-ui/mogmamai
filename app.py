...
# 세션 상태 초기화 (데이터가 날아가지 않게 함)
for key in ['texts', 'chat_log', 'm_name', 'm_mat', 'm_per', 'm_tar', 'm_det']:
    if key not in st.session_state:
        if key == 'texts': st.session_state[key] = {"인스타": "", "아이디어스": "", "스토어": ""}
        elif key == 'chat_log': st.session_state[key] = []
        else: st.session_state[key] = ""

# --- [로직 1: AI 자동 사진 보정 엔진 - Pillow 기반] ---
...
def ask_mog_ai(platform, user_in="", feedback=""):
    ...
    if platform == "인스타그램":
        ...
        system_p = f"{base_rules} [3️⃣ 상담소 고민 상담 전용 로직] 역할: 핸드메이드 작가들의 든든한 선배이자 다정한 동료 '모그 AI'. 규칙: 엄마의 고민에 깊이 공감해주고, 실질적인 도움(이름 짓기, 답장 문구 등)을 줄 것. 마무리: 항상 작가님의 활동을 진심으로 응원하는 따뜻한 격려 멘트 필수."

    if feedback:
        u_content = f"기존 글: {user_in} / 수정 요청사항: {feedback} / 위 요청을 반영해서 다정하게 다시 써주셔요🌸"
    else:
        info = f"작품명:{st.session_state.m_name}, 소재:{st.session_state.m_mat}, 정성:{st.session_state.m_det}"
...
if st.button("💾 이 작품 정보 창고에 저장하기"):
    try:
        ...
        st.error(f"저장 중에 오류가 났어요. 오류내용: {e}")
