...
# 라이브러리가 요구하는 형식을 만족시키기 위해 Secrets 정보를 임시 JSON 파일로 만듭니다.
client_secrets = {
    "web": {
...
# 현재 경로에 임시 비밀번호 파일 생성
secrets_file_path = os.path.join(os.getcwd(), "client_secrets.json")
with open(secrets_file_path, "w") as f:
    json.dump(client_secrets, f)

try:
    # 'secret_key'가 아닌 최신 버전 규격인 'cookie_key'를 사용합니다.
    auth = Authenticate(
        client_secrets_file=secrets_file_path,
        cookie_name="mom_ai_login_cookie",
        cookie_key=st.secrets.get("AUTH_SECRET_KEY", "mog_secret_key_12345"),
        cookie_expiry_days=30
    )
except TypeError:
    # 혹시라도 인자 이름 없이 순서대로 받는 구버전일 경우를 대비
    auth = Authenticate(
        secrets_file_path,
        "mom_ai_login_cookie",
        st.secrets.get("AUTH_SECRET_KEY", "mog_secret_key_12345"),
        30
    )
...
st.markdown("""
    ...
    .stButton>button { 
        ...
        color: white !important; font-weight: bold; font-size: 22px !important; 
    }
    .result-card { 
