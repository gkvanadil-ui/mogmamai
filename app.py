import streamlit as st
import pandas as pd
from streamlit_gsheets import GSheetsConnection
import openai

# 1. 페이지 설정 (넓게 보기 설정)
st.set_page_config(page_title="모그 AI 비서", layout="wide", page_icon="🌸")

# --- ✨ UI/UX: 칸이 절대 섞이지 않게 만드는 강력한 스타일 ---
st.markdown("""
    <style>
    @import url('https://fonts.googleapis.com/css2?family=Noto+Sans+KR:wght@300;400;700&display=swap');
    
    html, body, [data-testid="stAppViewContainer"] {
        background-color: #FCF9F6;
        font-family: 'Noto Sans KR', sans-serif;
    }

    /* 각 입력 칸 사이의 간격을 넓히고 테두리를 강조 */
    .stTextInput, .stTextArea {
        margin-bottom: 20px !important;
    }
    
    /* 입력창 라벨(제목) 글씨 크기 키우기 */
    label p {
        font-size: 20px !important;
        font-weight: bold !important;
        color: #8D6E63 !important;
    }

    /* 입력창 내부 디자인 */
    .stTextInput input, .stTextArea textarea {
        font-size: 18px !important;
        border-radius: 12px !important;
        border: 2px solid #D7CCC8 !important;
        padding: 15px !important;
    }

    /* 버튼 디자인 */
    .stButton>button {
        width: 100%; border-radius: 15px; height: 3.5em;
        background-color: #8D6E63 !important; color: white !important;
        font-weight: bold; font-size: 18px !important;
    }

    /* 제목 스타일 */
    h1 { color: #8D6E63 !important; text-align: center; }
    h2 { color: #A1887F !important; padding-top: 20px; }
    </style>
    """, unsafe_allow_html=True)

# 2. 필수 데이터 및 연결 설정
api_key = st.secrets.get("OPENAI_API_KEY")
if 'texts' not in st.session_state: st.session_state.texts = {"인스타": "", "아이디어스": "", "스토어": ""}
if 'refined' not in st.session_state: st.session_state.refined = {"인스타": "", "아이디어스": "", "스토어": ""}
if 'chat_history' not in st.session_state: st.session_state.chat_history = []

# --- 3. 메인 화면: 상세 입력 섹션 (확실한 분리) ---
st.title("🌸 모그 작가님 AI 비서")

st.header("1️⃣ 작품 정보를 한 칸씩 채워주세요")

# [중요] 각 항목을 확실하게 나누어 배치합니다.
row1_col1, row1_col2 = st.columns(2)
with row1_col1:
    name = st.text_input("📦 작품 이름", value=st.session_state.get('name', ''), placeholder="예: 빈티지 튤립 뜨개 파우치", key="name_input")
with row1_col2:
    material = st.text_input("🧵 사용한 소재", value=st.session_state.get('material', ''), placeholder="예: 순면사, 린넨 안감", key="mat_input")

row2_col1, row2_col2 = st.columns(2)
with row2_col1:
    period = st.text_input("⏳ 제작 소요 기간", value=st.session_state.get('period', ''), placeholder="예: 주문 확인 후 3일 이내", key="per_input")
with row2_col2:
    target = st.text_input("🎁 추천 선물 대상", value=st.session_state.get('target', ''), placeholder="예: 친구 생일, 나를 위한 선물", key="tar_input")

# 상세 설명은 넓게 한 줄을 다 씁니다.
keys = st.text_area("✨ 정성 포인트와 상세 설명", value=st.session_state.get('keys', ''), placeholder="작가님의 정성이 들어간 부분이나 작품의 이야기를 자유롭게 적어주세요.", height=150, key="key_input")

# 입력값 저장
st.session_state.name = name
st.session_state.material = material
st.session_state.period = period
st.session_state.target = target
st.session_state.keys = keys

st.divider()

# --- 4. 기능 탭 시작 ---
tabs = st.tabs(["✍️ 판매글 쓰기", "📸 사진 보정법", "💬 고민 상담소", "📂 작품 창고"])

# [함수: AI 글쓰기]
def process_mog_ai(guide):
    if not api_key: return "API 키가 없어요🌸"
    client = openai.OpenAI(api_key=api_key)
    info = f"작품명:{name}, 소재:{material}, 기간:{period}, 대상:{target}, 상세설명:{keys}"
    prompt = f"당신은 작가 모그입니다. 다음 정보를 바탕으로 {guide['name']} 판매글을 작성하세요. 말투는 다정하게, 특수기호 ** 금지.\n[정보] {info}\n[스타일] {guide['desc']}"
    try:
        response = client.chat.completions.create(model="gpt-4o", messages=[{"role": "user", "content": prompt}])
        return response.choices[0].message.content.replace("**", "").replace("*", "").strip()
    except: return "연결이 잠시 끊겼어요🌸"

with tabs[0]: # 판매글 쓰기
    st.write("#### 💡 아래 버튼을 누르면 글이 써집니다.")
    c1, c2, c3 = st.columns(3)
    if c1.button("📸 인스타그램"): st.session_state.texts["인스타"] = process_mog_ai({"name": "인스타그램", "desc": "감성 일기 스타일"})
    if c2.button("🎨 아이디어스"): st.session_state.texts["아이디어스"] = process_mog_ai({"name": "아이디어스", "desc": "정성 강조 스타일"})
    if c3.button("🛍️ 스토어"): st.session_state.texts["스토어"] = process_mog_ai({"name": "스마트스토어", "desc": "정보 안내 스타일"})

    for k in ["인스타", "아이디어스", "스토어"]:
        if st.session_state.texts.get(k):
            st.info(f"📍 {k} 글이 완성되었어요^^")
            st.text_area(f"{k} 내용", value=st.session_state.texts[k], height=250, key=f"t_{k}")

with tabs[1]: # 보정법
    st.markdown("### 📸 사진 보정법 가이드")
    st.success("**네이버 스마트스토어 편집기**: 사진 올리고 [편집] - [자동보정] 클릭! (추천)")
    st.info("**포토(Fotor)**: [AI 원클릭 보정] 버튼 하나로 밝기 조절 끝!")
    st.link_button("👉 포토(Fotor) 바로가기", "https://www.fotor.com/kr/photo-editor-app/editor/basic")

with tabs[2]: # 상담소
    st.header("💬 작가님 고민 상담소")
    for m in st.session_state.chat_history:
        with st.chat_message(m["role"], avatar="🌸" if m["role"]=="user" else "🕯️"): st.write(m["content"])
    if pr := st.chat_input("작가님, 무엇이든 물어보세요..."):
        st.session_state.chat_history.append({"role": "user", "content": pr})
        with st.chat_message("user", avatar="🌸"): st.write(pr)
        with st.chat_message("assistant", avatar="🕯️"):
            ans = process_mog_ai({"name": "상담소", "desc": f"현실 조언 질문: {pr}"})
            st.write(ans)
            st.session_state.chat_history.append({"role": "assistant", "content": ans})
            st.rerun()

with tabs[3]: # 창고
    st.header("📂 나의 작품 창고 (구글 시트 연동)")
    conn = st.connection("gsheets", type=GSheetsConnection)
    df = conn.read(ttl=0)
    if st.button("✨ 지금 정보를 창고에 저장하기"):
        new_row = pd.DataFrame([{"name":name, "material":material, "period":period, "target":target, "keys":keys}])
        conn.update(data=pd.concat([df, new_row], ignore_index=True))
        st.success("안전하게 저장되었습니다! 🌸")
    st.divider()
    for i, r in df.iterrows():
        with st.expander(f"📦 {r['name']}"):
            st.write(f"소재: {r['material']} | 제작기간: {r['period']}")
            if st.button("📥 이 정보 다시 불러오기", key=f"l_{i}"):
                st.session_state.name, st.session_state.material = r['name'], r['material']
                st.session_state.period, st.session_state.target = r['period'], r['target']
                st.session_state.keys = r['keys']
                st.rerun()
